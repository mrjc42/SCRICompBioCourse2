...
This week we will introduce you to the experience of using high level interactive programming languages and specifically: to R.  In this class we will be introducing you to the language as a tool that you can use to explore data.  Next weeks course will then extend what we learned today so that you can actually get started with writing your own code and scripts.
...
```{bash, eval=FALSE}
cp /tools/sampledata/course3/course3Files.tar.gz .
```
...
Because R is a data centric statistical language it has a lot of fun ways for representing data.  And as we mentioned above, the vector is the most fundamental object that R has.  

#### Creating vectors

There are two common ways to create vectors. If you are creating a numeric vector, one simple way is to use the `:` operator. You specify the first digit and the last digit, and R will fill in the rest in increments of one.
...
Another common way to create vectors is by using the __concatenate__ function. It technically just combines vectors rather than creating them. But given that single elements are just vectors of length 1, it amounts to the same thing. Use this to create character vectors and vectors of other types:
...
You can also use the `seq` or `rep` functions to quickly create other vectors.


#### Introducing the `class` and `length` functions

So we already talked about how vectors are the most basic data structure in R.  What we didn't spend as much time on is that they come in different flavors or classes.  The fact that vectors come in different classes is important because vectors can only represent objects of a single type at any one time.  You can see what class an object is by passing it to the class() function like this:
...
Sometimes you want to put more than just one flavor of data into a vector-like object.  For those times you need a much more flexible kind of container.  And this is what lists are for.  Lets look an an example of a list:
...
As we discussed previously, lists in R can hold different kinds of objects in each of a list elements.  It's a useful analogy to think of list objects like a train, where each car can hold different kinds of content:

### Lists act like a train
![Diagram 1](TheListTrain.png)

But a direct consequence of having an object that can hold multiple different things is that you will need more than one way to subset information from it.  In other words, sometimes you will want to subset a list in order to produce a shorter list, and other times you will want to subset a list so that you extract the contents from one of the list elements (remove contents from one of the train cars)...

So in addition to the single bracket subsetting operator, list objects also support a double bracket subsetting operator.  This kind of operator is normally only used with list like objects such as `data.frame`s and `list`s.  The difference between the single and double bracket operator is in the type of data that is returned.  While the single bracket operator will try to return the same type of object as it was used on, the double bracket will return the type of thing that is 'in' the container.  Lets look again at our list example:
