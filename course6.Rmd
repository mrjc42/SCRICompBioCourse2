...
1. From the above location, copy the folder 'Course6Exercises' to your O drive. 
...
`\\childrens\research\ISInformaticsTransfer\CourseDownloads\Course6Exercises`
...
First, let's make a space to do our work. We are going to be making links to reference files, so we will need some local scratch space, so we will set up shop in the `/data` location.
...
mkdir data
cd data
cp /tools/sampledata/course6/rnaseq/data/* .
```
...
We have made many passing references to the use of regular expressions without going into any detail. Indeed, regular expressions could be a class all by itself. But they are extremely useful and we make use of them in this next section, so I will touch briefly on some important points. 

#### What are regular expressions
Regular expressions are merely a set of wild card symbols used for doing pattern searching. You use them when you want to match very general patterns. For example:

* Any digit: `\\d`
* Any letter, number or `_`: `\\w` 
* Any white space: `\\s`
* Any character: `.`

You can also add some modifiers to use in combination with the above:

* `+`: Look for the longest possible match of one or more of the preceeding characters
* `?`: Added after `+` to look for the shortest possible match
...
#### Using with search and replace
The real power from regular expressions comes with the ability to find a match, and capture portions of that match to use as a replacement term. 

* `()`: Capture the search results between the parenthesis
* `\\1`: Substitute the contents of capture here

In R, you would commonly use this in conjunction with the `gsub` (global substitution) command. `gsub` takes as its arguments

* a search pattern
* a substitution string
* a vector to search through

The function will search each element of the vector for a match to the pattern. If the match is successful, it will replace that element with the substituion string. 

Regular expressions are a handy way to extract useful bits of information from text strings, thus saving you the trouble of having to type in lots of information by hand and risk mistyping. Consider the following file samples:

```{r}
samples <- c('Sample1_Normal', 'Sample2_Tumor', 'Sample3_Tumor', 'Sample4_Normal')
samples
```

We can use `gsub` to easily extract both the sample names and the sample type into their own vectors:
```{r}
samplenames <- gsub('(Sample\\d).+', '\\1', samples)
samplenames

sampletypes <- gsub('Sample\\d_(.+)', '\\1', samples)
sampletypes
```

***
####  <span style="color:blue">__Exercise L4:__</span>
1. Open RStudio by pointing your Firefox browser at https://sideswiper:8787. From the Files pane (usually the lower right pane), browse to `/tools/sampledata/course6/exercises/` and open the file `ExerciseL4-L5.R`)
...
# R has 'merge' function that you can use to merge two data frames. However, it only works on two data frames at a time. If you want to merge a list of data frames, you can take advantage of the 'Reduce' function, which is supplied from a handy package called 'plyr'. Don't get fussed about the syntax. Take it on faith that it works.
...
# Subset the data frame to exclude the first five rows, and the first column.
...
```{bash, eval=FALSE}
cd $RNASEQ
cp /tools/sampledata/course6/exercises/ExerciseL1-L3.sh .
```
...
Creating the above script can be tedious and/or challenging when you are trying to repeat the same command on different files. This is where a good text editor and the judicious use of regular expressions can save you a lot of work. A full lesson on regular expressions is beyond the scope of this course. However, this bonus exercise should be sufficient to give you some practice with the general principles that we covered earlier, as well as a taste of why they are so useful and perhaps motivate you to learn more about them outside of this class. 

1. Open a good text editor. 

2. Open the file `HomeworkL7.sh` from the Course6Exercises folder that you obtained earlier. If you cannot find it, you can get it again from the following Departmental Share location:
`\\childrens\ISInformaticsTransfer\CourseDownloads\Course6Exercises`
...
1. Within RStudio, use the __Files__ pane (usually in the lower right corner) to find and open the following file: `/tools/sampledata/course6/exercises/HomeworkL8.R`
