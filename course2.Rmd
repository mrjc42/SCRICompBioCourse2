...
__`cut`__ If you just wanted to display a subset of the information, say the Dive, Date and Depth, you can use the `cut` command. Examine the man page for `cut` for details on usage. The `-f` flag specifies what columns, or fields to display
...
__`sort`__ If you want to put the output in a particular order, you can use the `sort` command. In our previous example, we can sort by the dive name:
```{bash, eval=FALSE}
