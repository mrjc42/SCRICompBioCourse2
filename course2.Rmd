...
author: "Sean, Marc, Bronson, Debbie"
date: "2/17/2016"
...
If you did not complete the homework, you can run this catchup script. This will ensure that your environment looks like mine before we start this class.
```{bash}
/tools/sampledata/course2/Homework1_catchup.sh
```
...
Whatever style you prefer, these all have one thing in common: no spaces.  You shouldn't use white space in file names when using Linux.  You technically can do it, but everyone agrees that it's not worth it since you have to escape all your white space characters:
...
Another common case with rm is when you have a directory and you want to remove it AND everything in it. Remember that complicated file structure we created last time inside `~/sandbox/ourData`? If you wanted to remove `ourData ` and everything in it, you can do it __recursively__.

__Recursion__ is useful principle that comes up in fields like computer science, mathematics, and logic. We won't go too deep into it, but you should be familiar with the flavor of the term. In this context, the idea is that you call a function that references itself in it's own definition. So for example I could have a function called `delete` whose definition could be

> Delete: Remove any files in this directory. If you find a subdirectory, execute `Delete`.

Often you will see commands with a flag to do something recursively, which allows you to execute the command on a directory, and all of it's subdirectories:
...
### How to avoid creating many redundant copies of important files: Using __`ln`__

Sometimes you will want to just create a shortcut to a file rather than actually making a copy. Perhaps it is a reference FASTA file that is very big, so making multiple copies of it isn't practical. In the following example, notice how big the `genome.fa` file is:
```{bash}
...
```{bash, eval=TRUE, echo=FALSE}
cd ~/sandbox/myData
ln -sf /tools/references/Homo_sapiens/UCSC/hg19/Sequence/WholeGenomeFasta/genome.fa myGenome.fa
```
...
Sometimes less is more.  And when you are using Linux suddenly `less` literally is a more advanced version of the `more` command.  The `more` command was long used for looking at very large text files.  And `less` does the same basic job as `more`.  Have a gigantic file that you can't even look at in excel?  `less` will probably open that file without even flinching.  Unlike text editors in windows and macs, `less` does not wait for the entire file to load into memory before showing you it's contents. Rather, it starts *paging* the file contents onto the screen so you can begin looking at it immediately. While you are doing that, it is quietly continuing to load the rest of the file into memory in case you want to see more of it. By now you should already be somewhat familiar with how `less` works, but we will repeat the basic syntax and keystrokes that you use with `less`: 
...
Note that `less` is great for looking at your files, and it's "safe" because it is read-only, meaning you can't change anything. If you need to edit the contents of a file, you will need to use a text editor.
...
cd /tools/sampledata/course2
cp *.seq ~/sandbox/myData/
cp *.pdb ~/sandbox/myData/
```
...
Again, displaying it on the screen is not so useful. What you would really like to do is save the output to a new file. For this, you use `>`, the redirection operator.

### Redirecting output to a file: Using __`>`__ and __`>>`__

If at any time you want to save the output from a command or set of commands, you would use the "redirection operator", or "bent pipe" which is also known as: `>`
...
1. Navigate to your home directory.
2. Using cat, concatenate FEC00007_1.seq and FEC00007_2.seq and save the output to a file called FEC00007.seq.
...
The find command is for finding files in the file system.  But it is actually VERY powerful because like everything else today it can be paired with the other toys in the Linux toy box.  The find command takes a couple of arguments. The 1st is the part of the directory to search in and the second is what to look for.  A common way to use find is to find a file with a specific name.  So to find a file named `bob.txt` in your local directory could could do this
...
Similar to fiding a file, you may want to know if a particular executable is available and ready to execute. The output `which` will either tell you the path of where the command lives or will tell you that it couldn't find it in any of the expected locations (meaning it either isn't installed or it isn't ready to be executed easily).
```{bash, error=TRUE}
which bwa
```
```{bash, error=TRUE}
...
Sometimes you will want a job to run and you already know that it's going to take several hours. When that happens, you might worry that if you log out your job will not complete. For this circumstance you will want to use the `nohup` command (or: no hang up).  You can use it to run a command in the background.  `nohup` will keep running that process until it either finishes or until someone kills it.  You can use `nohup` like this:
...
1. Copy the compressed archive file `/tools/sampledata/course2/homework2.tar.gz` into `~/sandbox/myData`
...
1.  The `/tools/references` directory contains reference sequence information for several model organisms. Create a link in `~/sandbox/myData` to the whole genome fasta file for the _D. melanogaster_ dm3 reference sequence. Call the link flyGenome.fasta.
...
2. Here you will discover four files with a `.diff` extension. These are outputs from a differential expression analysis. The analysis looked for differential expression based on gene region, coding sequences, promoter regions, or alternative splicing sites.
...
```{bash, eval=FALSE}
cd ~/sandbox/myData
cp /tools/sampledata/course2/homework2.tar.gz .
