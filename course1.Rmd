...
Most wouldn't guess that Citrix is actually being used by just shy of everyone. Almost all of the icons on your desktop, besides a select few, are populated when citrix loads. How you access citrix and how you save what you are working on makes the difference when it comes to citrix. Knowing how to use your citix can be the difference to saving an important document and having to rebuild from scratch. So we will go over some of the safest ways you can use citrix to your advantage!
...
* Running localy installed applications will always be faster than Citrix applications
...
cp Marrus_claudanielis.txt structure_1ema.pdb shaver_etal.csv nanodrop_abs.ndj \
    ~/sandbox/ourData/sharedData/ 
```

4. Alternative answer using Syntax 3 from man page.
```{bash, eval=FALSE}
cp -t ~/sandbox/ourData/sharedData/ \
Marrus_claudanielis.txt structure_1ema.pdb shaver_etal.csv nanodrop_abs.ndj
```

5. This should return an error. Because the name has a space in it, the command is exptecting to find two files, one named 'Optode' and another named 'run011.xls'. Because neither of those files exists, the command fails.
```{bash, eval=FALSE}
