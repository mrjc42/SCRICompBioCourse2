...
* Directories = folders
* Executabes = applications/programs
...
pwd
```
`pwd` means print working directory.  It will tell you where in the file system you are by giving you the *path* starting with root. Think of the path as a road map of directories and subdirectories that tell you how to get from root to your folder or file or executable.

In Linux a full directory path will start with root: `/`, use more `/`'s in between as separators for each subdirectory and will often end with another `/`. 
...
```{bash, eval=FALSE}
ls -l /tools/sampledata/examples
```
```{bash, echo=FALSE}
ls -l /tools/sampledata/examples | head -n 12
```
...
You may notice that the item `ctd` is colored differently. This is because it is a directory, as inticated by the leading character `d` of the permissions string.

You may also notice that the item `badtextfile.txt` is also colored differently. This is a hint that this item may be an exectuable, as indicated by the presence of the `x` characters in the permission string.
...
3. List the contents of this directory. You should see subdirectories for each user on the system. Are you allowed to see the contents of others directories?  How do you know?
...
5. Try transfering the file `Optode run011.xls`. What was the result? Try copying again, but this time use this modified file name: `Optode\ run011.xls`. What does this tell you about using spaces in file names?
...
2. Use the table to explore the contents. Try scrolling forwards and backwords through the file. Use the proper keystrokes to skip pages forward and backward. Search for the occurence of the string "ASP" to find the coordinates of the aspartate residues in the structure.
...
## 9. (Should result in an error because 'shared' does not exist yet.)
...
## 2.
cp /tools/sampledata/examples/FEC00001_1.seq ~/sandbox/myData
ls ~/sandbox/myData
...
## 4.
cd /tools/sampledata/examples
cp Marrus_claudanielis.txt structure_1ema.pdb shaver_etal.csv nanodrop_abs.ndj ~/sandbox/ourData/sharedData/ 

## alternative answer
cp -t ~/sandbox/ourData/sharedData/ Marrus_claudanielis.txt structure_1ema.pdb shaver_etal.csv nanodrop_abs.ndj

## 5. 
cp Optode run011.xls ~/sandbox/ourData/sharedData/

## This should return an error. Because the name has a space in it, the command is exptecting to find two files, one named 'Optode' and another named 'run011.xls'. Because neither of those files exists, the command fails.

cp Optode\ run011.xls ~/sandbox/ourData/sharedData/

## Using the "\" before a space tells the command that the space is part of the file name. This is annoying though, so as a general rule we can see that putting spaces in file names is a bad idea.
```
